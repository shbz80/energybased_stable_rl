...
# pure icnn itr 49
exp_name = 'elitebook/cem_energybased_block2d'
filename = base_filename + '/' + exp_name + '/' + 'itr_' + str(49) + '.pkl'
infile = open(filename, 'rb')
ep_data = pickle.load(infile)
infile.close()
epoch = ep_data['stats'].last_episode
...
# icnn + quad itr0
exp_name = 'cem_energybased_block2d_9'
filename = base_filename + '/' + exp_name + '/' + 'itr_' + str(0) + '.pkl'
infile = open(filename, 'rb')
ep_data = pickle.load(infile)
infile.close()
...
#                 cmap='viridis', edgecolor='none',alpha=0.7)
pos = epoch[14]['observations'][:, :2]
ax1.plot(pos[:,0], pos[:,1], color='g',linewidth=2,label='Itr 49')
...
ax1.legend(loc='upper left', bbox_to_anchor=(-2, 1.3),frameon=False,ncol=3,prop={'size': 10})
