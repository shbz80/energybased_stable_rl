...
class MLPModuleJac(MultiHeadedMLPModule):
    """MLP Model.

    A Pytorch module composed only of a multi-layer perceptron (MLP), which
    maps real-valued inputs to real-valued outputs.

    A new method is added that is like forward() but also takes in the parameters as
    arguments. This is necessary for jacobian computation.
    ...
    # def forward_func(self, params, input_val):
    #     x = input_val
    #     for param in params:
    #
    #         x = layer(x)
    #
    #     return [output_layer(x) for output_layer in output_layers]
